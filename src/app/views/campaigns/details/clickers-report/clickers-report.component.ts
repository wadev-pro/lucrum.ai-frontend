...
@Component({
  selector: 'app-campaign-details-clickers-report',
  templateUrl: './clickers-report.component.html',
  styleUrls: ['./clickers-report.style.css'],
  animations: egretAnimations,
})
export class ClickersReportComponent implements OnInit, OnDestroy {
  ...
  public pagination: TablePagination = {
    length: initialState.clickMeta.total,
    pageIndex: initialState.clickFilter.page,
    pageSize: initialState.clickFilter.per_page,
    previousPageIndex: 0,
  };
  ...
  constructor(
    ...
    this.filter$ = this.store$.select(clickFilterSelector);
    this.meta$ = this.store$.select(clickMetaSelector);
  ...
  loadData() {
    ...
    this.store$.dispatch(new CampaignStatisticsActions.GetClick(payload));
  }
  ...
  updateFilter(data) {
    ...
    this.store$.dispatch(
      new CampaignStatisticsActions.UpdateClickFilter(updated_filter)
    );
  }
