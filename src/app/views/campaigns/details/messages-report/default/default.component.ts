...
import {
  ...
  messageSentMetaSelector,
} from 'app/store/campaign-statistics/campaign-statistics.selectors';

@Component({
  selector: 'app-campaign-details-messages-report-default',
...
export class DefaultComponent implements OnInit, OnDestroy {
  ...
  public pagination: TablePagination = {
    length: initialState.messageSentMeta.total,
    pageIndex: initialState.messageSentFilter.page,
    pageSize: initialState.messageSentFilter.per_page,
    previousPageIndex: 0,
  };
  ...
  constructor(
    ...
    this.filter$ = this.store$.select(messageSentFilterSelector);
    this.meta$ = this.store$.select(messageSentMetaSelector);
  ...
  loadData() {
    ...
    this.store$.dispatch(new CampaignStatisticsActions.GetMessageSent(payload));
  }
  ...
  updateFilter(data) {
    ...
    this.store$.dispatch(
      new CampaignStatisticsActions.UpdateMessageSentFilter(updated_filter)
    );
  }
