...
@Component({
  selector: 'app-campaign-details-statistics-carrier-statistics',
  templateUrl: './carrier-statistics.component.html',
  styleUrls: ['./carrier-statistics.style.css'],
  animations: egretAnimations,
})
export class CarrierStatisticsComponent implements OnInit, OnDestroy {
  ...
  public pagination: TablePagination = {
    length: initialState.carrierMeta.total,
    pageIndex: initialState.carrierFilter.page,
    pageSize: initialState.carrierFilter.per_page,
    previousPageIndex: 0,
  };
  ...
  constructor(
    ...
    this.filter$ = this.store$.select(carrierFilterSelector);
    this.meta$ = this.store$.select(carrierMetaSelector);
  ...
  loadData() {
    ...
    this.store$.dispatch(new CampaignStatisticsActions.GetCarrier(payload));
  }
  ...
  updateFilter(data) {
    ...
    this.store$.dispatch(
      new CampaignStatisticsActions.UpdateCarrierFilter(updated_filter)
    );
  }
